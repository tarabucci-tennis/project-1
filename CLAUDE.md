...
- **`docker logs -f --tail 0` on the droplet while Tara clicked Add These Players → no `POST /teams/:id/paste_roster` ever appeared.** Only GETs to other pages. That narrowed the problem to "the form submission isn't leaving the browser at all" — i.e., client-side, not server-side.

**5. PR #58 — Paste Roster rewritten as plain `<form>` + JS onsubmit alert diagnostic**

- Rewrote the paste_roster form: dropped `form_with`, used a hand-written `<form action="..." method="post" accept-charset="UTF-8" data-turbo="false">`, explicit `<input type="hidden" name="authenticity_token" value="<%= form_authenticity_token %>">`, plain `<button type="submit">`.
- Added `onsubmit="alert('Submitting ' + roster.length + ' chars to ' + this.action); return true;"` as an intentional, temporary diagnostic — if the form fires, Tara sees an alert. If she clicks and no alert appears, the button is DOM-orphaned from the form.
- **Tara never actually tested PR #58 in the end** — she pivoted to the direct-database workaround below because the match day was advancing and she just needed the test team populated. The diagnostic is still sitting there in `app/views/teams/show.html.erb` for next session.

**6. Pivot: direct Rails runner scripts for Baby Got Backhands (test team)**

Rather than keep debugging the form, Tara asked Claude to just add the roster and schedule directly. Built two `docker exec -i project-1 bin/rails runner - <<'RUBY' ... RUBY` one-liners she pasted into the droplet terminal:

- **Roster script** — 23 players with `team.members.any? { |u| u.name.downcase == n.downcase }` idempotent check, `User.where("LOWER(name) = ?", ...).first || User.create!` to reuse placeholders without duplicating, then `TeamMembership.create!` with `role: "player"`. Output shows "Added N new players" and "Already on team (M)" so it's safe to re-run. (First version had 24 names with Chloe Seaver; Tara later sent an updated list of 23 without her.)
- **Schedule script** — 9 matches (first pass was 8, missed the 6/12 Netchix match which Tara caught on review). Safety check: `abort "SAFETY: N match(es) have results entered. Refusing to delete."` if any existing match has `result` set. Otherwise `team.matches.destroy_all` and recreate from the 9-match data array. Each match gets `home_away:` + `notes: "Home match"/"Away match"` so the existing view logic (`match.notes&.include?("Home")`) renders the right badge. Also fills in missing `home_court`, `league_category`, `section`, `rating`, `gender`, `team_type`, `season_name` if blank.
- Both scripts work cleanly via docker-exec-heredoc → no need for browser or form at all. **This is now the documented workaround for "the app's form is broken but I need the data in the DB anyway."**

**7. Scraping vs Google Sheets — Tara's decision for the spring season**

- Tara asked "can you scrape all my teams' data?" Claude laid out the honest cost: 3 different websites (TennisLink / Inter-Club / Del-Tri) each with different HTML, TennisLink requires USTA OAuth (credentials stored on droplet, session cookie management, OAuth flow), plus scheduled job + error handling. Multi-session project. Not buildable today.
- Tara pivoted: **"How can I get the data entered into my spreadsheets to connect to my site?"** Much better path. The plumbing already exists — `PagesController#stats_test` fetches a public Google Sheet as CSV via the `export?format=csv` URL with no API key. Reusable pattern.
- **Proposed design:** one public Google Sheet per team with tabs for Roster / Schedule / Results in a fixed column format. `TennisTeam` gets a `google_sheet_url` column. A "Sync from Sheet" button on the team page (or nightly Solid Queue job) fetches + upserts. Tara edits the sheet from any device; Court Report reflects it. No OAuth, no credentials, no scraping, works across every league equally.
- **Not yet implemented.** Proposed, Tara endorsed in principle, deferred until after today's layout work is stable. Logged in the pending section below.

**8. Home page CSS polish — PRs #59, #60, #61, #62, #63, #64 (many iterations)**

Tara wanted the My Teams home page to look "very professional, clean and balanced." What should have been one or two small CSS tweaks turned into six PRs because Claude can't see the rendered result in a browser and kept getting the spec slightly wrong or introducing regressions. Details below so we don't repeat the same loop next time.

- **PR #59 — Dark calendar + darker header + equal columns.** Claude interpreted Tara's "I'd like the calendar and left games to be even" as "make both columns same width AND same height," and layered `align-items: stretch` + `.cr-dash-cal { height: 100%; flex }` + `.cr-dash-cal-grid { flex: 1 1 auto }`. Also dark-themed the whole calendar. New CSS variables: `--cr-header-bg: #030a1a` (header), `--cr-dark-panel: #0a1a33` / `--cr-dark-panel-soft: #142545` / `--cr-dark-border: rgba(201, 162, 74, 0.18)` (calendar surfaces).
- **PR #60 — Revert calendar dark theme, match days → dark blue.** Tara clarified: she did NOT want the whole calendar dark. She just wanted the tiles that were ALREADY black (match days) changed to dark blue. Reverted every `.cr-dash-cal*` surface back to the light palette from before PR #59, kept the darker header bg and the equal-width grid. Changed `.cr-dash-cal-day-match` from `#1a1a1a` to `var(--cr-black)` (#0a1628) which is the navy brand color — reads as dark blue rather than pure black. Hover bumps to `var(--cr-black-soft)` (#111f36). The unused `--cr-dark-*` variables were left in `:root` because they don't cost anything.
- **PR #61 — "Coming Up" title top alignment.** Tara noticed "Coming Up" was slightly above "April 2026" at the top of the row. Root cause: `.cr-my-week-title` is an h2 with no explicit `margin-top`, so it got the browser default (~0.83em), pushing the text down inside the left column while the calendar panel on the right had no such offset. Fix: `margin-top: 0` + `padding-top: 14px` to match the calendar panel's internal top padding. Got them level.
- **PR #62 — Fix calendar blowing up past its column.** Tara reported the calendar was way oversized after PR #59's stretch hack shipped. Root cause: `.cr-dash-cal-day { aspect-ratio: 1 / 1 }` + `align-items: stretch` → grid grew tall → day cells' height grew → aspect-ratio forced matching width → calendar exploded out of its column. Reverted stretch + flex + flex-column + `flex: 1 1 auto` back to natural heights. Kept equal widths and top alignment from #59/#61.
- **PR #63 — Fix horizontal page overflow.** Tara reported the whole page was horizontally scrolling — header truncated to "RT REPORT", left cards showing only their right halves, calendar rightmost column cut off. Root cause: `.cr-dash-cal-day`'s `aspect-ratio: 1 / 1` STILL caused problems even without stretch — on a 1440px+ Mac viewport, the 1fr calendar column was wide enough that each square cell wanted to be ~160px × 160px, and 7 cells + gaps exceeded the column's share, forcing `.cr-home-top` wider than `.cr-main`, forcing horizontal scroll. The `minmax(0, 1fr)` should have prevented this but didn't. Fix: **remove `aspect-ratio` entirely**, use explicit `.cr-dash-cal-day { height: 44px; min-width: 0 }`. Also hard-capped calendar column at `minmax(260px, 400px)`, added `overflow: hidden` to `.cr-home-top` as a belt-and-suspenders safety net, and `max-width: 100%; margin: 0 auto` on `.cr-dash-cal`.
- **PR #64 — Equal-width columns, centered, calendar top-aligned with first card (not "Coming Up").** Tara clarified the target layout: she wanted cards and calendar to be the SAME WIDTH, and the calendar panel's top edge to be level with the FIRST CARD's top (not with the "Coming Up" h2). Fix: `grid-template-columns: minmax(260px, 400px) minmax(260px, 400px)` (both columns same max), `max-width: 824px; margin: 0 auto; justify-content: center` on `.cr-home-top` (centered 824px block), and `margin: 38px auto 0` on `.cr-dash-cal` (push calendar down 38px ≈ h2 height + margin so the panel top lines up with first card top). Reverted PR #61's `padding-top: 14px` on the h2 — that was no longer needed since the calendar is the thing moving, not the title.

**Open at end-of-session:**

- **Layout still not quite right.** After PR #64 deployed, Tara said the alignment is still off and asked to have the cards + calendar "level" — unclear if the 38px margin-top on `.cr-dash-cal` is wrong for her actual rendered h2 height, or if she means something different by "level" than top-aligned first-card-to-calendar-panel-top. Next session: either (a) match h2 + margin exactly by using invisible h2 spacer on the right side via ERB, or (b) just remove the h2 from the grid entirely and put "Coming Up" as a title above the whole row.
- **Coming Up list needs a 2-week cap.** Tara asked "please only list two weeks of games at the top left under coming up." Verified `teams_controller#index` already filters `@upcoming_matches` to `14.days.from_now.end_of_day` — so the window is already 14 days. Either (a) the filter isn't taking effect in production somehow, (b) she's seeing matches more than 14 days out for a different reason (multiple teams with matches clustered early), or (c) she actually wants a MAX COUNT (e.g. 5) rather than a time window. Not yet addressed. Next session: verify the query on her real data, add `.limit(5)` as a safety.
- **Paste Roster root cause still unknown.** PR #58's plain-form-with-alert diagnostic is deployed but Tara never tested it before pivoting to direct-DB. Next session, if she's willing to test it once in a Private tab, the alert popup + a simultaneous `docker logs -f` will pin the root cause in under a minute.
- **Google Sheets integration not built.** Design is proposed (public-CSV-per-team, reuse the `stats_test` fetch pattern, "Sync from Sheet" button on team page), Tara is on board in principle. Build is a future session.
...
Working branch: `claude/mobile-nav-improvements-b36wW`. Six feature commits + a CLAUDE.md write-up were opened as **PR #83** at end of session and **squash-merged to main** (`4189487`). Auto-deploy picked it up and Tara verified the site comes up and the new features render. Six commits were fast-forward on top of `8fba13c` (pre-session `origin/main`).
...
<!-- Session 14 PR opened as #83 and squash-merged to main (4189487) before end of session. Auto-deployed; Tara verified. -->
